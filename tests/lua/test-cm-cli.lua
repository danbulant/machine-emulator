...
local function test_ram_dtb()
    ...
    assert(cfg.ram.backing_store.data_filename:find("linux%.bin"), "--ram=: filename mismatch")

    cfg = config_for({ "--dtb=data_filename:" .. dtb_tmp })
    assert(cfg.dtb.backing_store.data_filename:find("%d+%.dtb"), "--dtb=: filename mismatch")

    cfg = config_for({ "--processor=data_filename:" })
    assert(cfg.processor.backing_store.data_filename == "", "--processor=: data_filename not set")

    cfg = config_for({ "--pmas=data_filename:" })
    assert(cfg.pmas.backing_store.data_filename == "", "--pmas=: data_filename not set")

    cfg = config_for({ "--uarch-ram=data_filename:" })
    assert(cfg.uarch.ram.backing_store.data_filename == "", "--uarch-ram=: data_filename not set")
    ...
    if uf then
        ...
        assert(cfg.uarch.ram.backing_store.data_filename:find("uarch%-ram%.bin"), "--uarch-ram-image: filename not set")
    end
end
...
local function test_flash_drive()
    ...
    for _, fd in ipairs(cfg.flash_drive) do
        if fd.label == "root" then
            assert(fd.read_only, "--flash-drive=label:root,read_only: read_only not set")
        end
    end
    assert(not cfg.dtb.bootargs:find("pmem0 rw"), "--flash-drive=label:root,read_only: bootargs still mounts rw")
    ...
    assert(#cfg.flash_drive >= 2, "unlabeled --flash-drive: entry not added")
    ...
    assert(#cfg.flash_drive >= 2, "unlabeled --flash-drive mount:true: entry not added")
    ...
    assert(not cfg.dtb.init:find("/mnt/empty"), "flash-drive no-data/no-mke2fs: unexpected mount in dtb.init")
...
local function test_nvram()
    ...
    assert(shared_count == 1, "--nvram repeated label: expected 1 entry, got " .. shared_count)
end
...
local function test_rollup_advance_inspect()
    ...
    assert(out00.payload == "hello", "output for input 0 not echoed correctly")
    local out10 = evmu.decode_calldata(voucher_sig, read_bin(dir .. "/out-1-0.bin"), "raw")
    assert(out10.payload == "world", "output for input 1 not echoed correctly")
